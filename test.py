...
from utils.general import (
    ...
    scale_coords, xyxy2xywh, clip_coords, plot_images, plot_test_images, xywh2xyxy, box_iou, output_to_target, ap_per_class)
...
def test(data,
    ...
    if not training:
        ...
        root = data['root']
        path = data['test'] if opt.task == 'test' else data['test_emb']  # path to val/test images
        dataloader = create_dataloader(root, path, imgsz, batch_size, model.stride.max(), opt,
    ...
    for batch_i, (img, targets, paths, shapes) in enumerate(tqdm(dataloader, desc=s)):
        ...
        with torch.no_grad():
            # Run model
            t = time_synchronized()
            inf_out, train_out_p, train_out_pemb = model(img, augment=augment)  # inference and training outputs
            t0 += time_synchronized() - t

            # Compute loss
            if training:  # if model has loss hyperparameters
                loss += compute_loss([x.float() for x in train_out_p], [x.float() for x in train_out_pemb],
                                     targets, model)[1][:4]  # GIoU, obj, cls, lid

            # Run NMS
            t = time_synchronized()
            output = non_max_suppression(inf_out, conf_thres=conf_thres, iou_thres=iou_thres,
                                         merge=merge, emb_dim=emb_dim)
        ...
        if  batch_i < 1:
            ...
            plot_test_images(img, output_to_target(output, width, height), paths, str(f), names)  # predictions
    ...
    # Print speeds
    t = tuple(x / seen * 1E3 for x in (t0, t1, t0+t1)) + (imgsz[0], imgsz[1], batch_size)
    if not training:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--emb_dim', action='store_true', help='dim of reid prediction', default=256)
