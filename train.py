...
from utils.general import (
    ...
    compute_loss, plot_images, fitness, strip_optimizer, plot_results,
...
hyp = {'lr0': 0.01,  # initial learning rate (SGD=1E-2, Adam=1E-3)
       ...
       'giou': 0.05,  # GIoU loss gain       # 0.075
       'cls': 0.5,  # cls loss gain
       'cls_pw': 1.0,  # cls BCELoss positive_weight
       'obj': 1.0,  # obj loss gain (scale with pixels)          # 1.35
       ...
       'lid': 0.0037,
       's_det': -1.58,  # uncertainty of detection    # -1.65
       's_id': -1.16}   # uncertainty of reid         # -1.15
...
def train(hyp, opt, device, tb_writer=None):
    ...
    # Trainloader
    dataloader, dataset = create_dataloader(root_path, train_path, imgsz, batch_size, gs, opt, hyp=hyp,
                                            augment=True, cache=opt.cache_images, rect=opt.rect,
                                            local_rank=rank, world_size=opt.world_size)
    # Testloader
    testloader = create_dataloader(root_path, test_path, imgsz_test, total_batch_size, gs, opt, hyp=hyp,
    ...
    if rank in [0, -1]:
        print('Image sizes {} train, {} test'.format(str(imgsz), str(imgsz_test)))
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        if rank in [-1, 0]:
            print(('\n' + '%10s' * 8 + '%13s') % ('Epoch', 'gpu_mem', 'GIoU', 'obj', 'cls', 'lid', 'total', 'targets', 'img_size'))
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            if opt.multi_scale and random.random() < 0.5:
                candidate_shapes = [[608, 1088], [480, 864], [320, 576], [512, 960], [384, 640]]
                curr_shapes = candidate_shapes[random.randint(0, 4)]
                imgs = F.interpolate(imgs, size=curr_shapes, mode='bilinear', align_corners=False)
            ...
            with amp.autocast(enabled=cuda):
                ...
                # Loss
                loss, loss_items = compute_loss(pred_detect, pred_emb, targets.to(device), model)  # scaled by batch_size
                if rank != -1:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', type=str, default='weights/new.pt', help='initial weights path')
