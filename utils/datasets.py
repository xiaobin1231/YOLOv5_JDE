...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, root, paths, img_size=(1088, 608), batch_size=16, augment=False, hyp=None, rect=False,
        ...
        try:
            ...
            for ds, path in paths.items():
                with open(path, 'r') as file:
                    ...
                    self.name_files.extend(self.img_files[ds])
            ...
            self.nID = int(last_index + 1)  ### 所有数据集id总个数
            self.nds = [len(x) for x in self.img_files.values()]  ### 每个数据集里面包含多少张图
            self.cds = [sum(self.nds[:i]) for i in range(len(self.nds))]  ### 前i个数据集总共包含多少张图
            self.nF = sum(self.nds)  ### 　一共有多少张图
            self.width = img_size[0]
            self.height = img_size[1]

            bi = np.floor(np.arange(self.nF) / batch_size).astype(np.int)
            ...
            self.mosaic_border = [-img_size[1] // 2, -img_size[0] // 2]  ### [h, w]
            self.stride = stride

            # Check cache
            flag = 'train' if self.augment else 'test'
            cache_path = str(Path(root)) + '/{}.cache'.format(flag)       # cached labels
            ...
            # Get labels
            labels, shapes = zip(*[cache[x] for x in self.name_files])
        ...
        print('=' * 80)
        print('{} dataset summary'.format(flag))
        print(self.tid_num)
    ...
    def cache_labels(self, path='labels.cache'):
        ...
        x = {}  # dict
        pbar = tqdm(zip(self.img_files), desc='Scanning images', total=len(self.img_files))
        for ds in pbar:
            try:
                for img, label in zip(self.img_files[ds[0]], self.label_files[ds[0]]):
                    l = []
                    ...
                    if os.path.isfile(label):
                        with open(label, 'r') as f:
                            ...
                            for i, _ in enumerate(l):
                                if l[i, 1] > -1:
                                    l[i, 1] += self.tid_start_index[ds[0]]
                    if len(l) == 0:
    ...
    def __getitem__(self, index):
        ...
        return torch.from_numpy(img), labels_out, self.name_files[index], shapes
...
def load_mosaic(self, index):
    ...
    for i, index in enumerate(indices):
        ...
        x = self.labels[index]
        labels = x.copy()
        original_areas = w * x[:, 4] * h * x[:, 5]

        if x.size > 0:  # Normalized xywh to pixel xyxy format
            inds = (original_areas != 0)
            labels[:, 2] = np.clip(w * (x[:, 2] - x[:, 4] / 2) + padw, x1a, x2a)
            labels[:, 3] = np.clip(h * (x[:, 3] - x[:, 5] / 2) + padh, y1a, y2a)
            labels[:, 4] = np.clip(w * (x[:, 2] + x[:, 4] / 2) + padw, x1a, x2a)
            labels[:, 5] = np.clip(h * (x[:, 3] + x[:, 5] / 2) + padh, y1a, y2a)
            curr_areas = (labels[:, 4] - labels[:, 2]) * (labels[:, 5] - labels[:, 3])
            area_ratio = curr_areas[inds] / original_areas[inds]
            labels = labels[inds][area_ratio > 0.3]
    ...
    if len(labels4):
        ...
        np.clip(labels4[:, 2], 0, 2 * sw, out=labels4[:, 2])  # use with random_affine
        np.clip(labels4[:, 4], 0, 2 * sw, out=labels4[:, 4])
        np.clip(labels4[:, 3], 0, 2 * sh, out=labels4[:, 3])
        np.clip(labels4[:, 5], 0, 2 * sh, out=labels4[:, 5])
...
def random_perspective(img, targets=(), degrees=10, translate=.1, scale=.1, shear=10, perspective=0.0, border=(0, 0)):
    ...
    if n:
        ...
        i = (w > 4) & (h > 4) & (area / (area0 + 1e-16) > 0.25) & (ar < 10)
