...
def compute_loss(p, p_emb, targets, model):  # predictions, targets, model
    device = targets.device
    if model.training:
        emb_scale = math.sqrt(2) * math.log(model.module.nID - 1) if model.module.nID > 1 else 1
        classifier = model.module.classifier
    else:
        emb_scale = math.sqrt(2) * math.log(model.nID - 1) if model.nID > 1 else 1
        classifier = model.classifier
    lcls, lbox, lobj, lid, loss = torch.zeros(1, device=device), torch.zeros(1, device=device), torch.zeros(1, device=device),\
                                  torch.zeros(1, device=device), torch.zeros(1, device=device)
    tcls, tbox, indices, anchors, tids = build_targets(p, targets, model)  # targets
    h = model.hyp  # hyperparameters
    s_det = nn.Parameter(h['s_det'] * torch.ones(1)).to(device)
    s_id = nn.Parameter(h['s_id'] * torch.ones(1)).to(device)

    # Define criteria
    BCEcls = nn.BCEWithLogitsLoss(pos_weight=torch.Tensor([h['cls_pw']])).to(device)
    BCEobj = nn.BCEWithLogitsLoss(pos_weight=torch.Tensor([h['obj_pw']])).to(device)
    IDLoss = nn.CrossEntropyLoss(ignore_index=-1).to(device)
    ...
    for i, pi in enumerate(p):  # layer index, layer predictions
        ...
        if n:
            ...
            curr_tids, _ = tids[i].max(1)
            curr_tids = curr_tids[emb_mask]

            embedding = p_emb[i][emb_mask]
            embedding = emb_scale * F.normalize(embedding)
            if model.training:
                logits = classifier(embedding).contiguous()
            else:
                logits = classifier(embedding.type(torch.half)).contiguous()
            lid += IDLoss(logits, curr_tids.squeeze())
    ...
    loss = torch.exp(-s_det) * (lbox + lobj + lcls) + torch.exp(-s_id) * lid + (s_det + s_id)
    loss *= 0.5
    return loss * bs, torch.cat((lbox, lobj, lcls, lid, loss)).detach()
...
def build_targets(p, targets, model):
    ...
    for i in range(det.nl):
        tid_lvl = torch.LongTensor(p[i].shape[0], na, p[i].shape[2], p[i].shape[3], 1).fill_(-1).to(device=targets.device)
        ...
        tid_lvl[b, a, gj, gi] = id_gt.unsqueeze(1)
        tbox.append(torch.cat((gxy - gij, gwh), 1))  # box
        anch.append(anchors[a])  # anchors
        tcls.append(c)  # class
        tid.append(tid_lvl)

    return tcls, tbox, indices, anch, tid


def non_max_suppression(prediction, conf_thres=0.1, iou_thres=0.6, merge=False, classes=None, agnostic=False, emb_dim=256):
    ...
    nc = prediction[0].shape[1] - 5 - emb_dim  # number of classes
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        if multi_label:
            ...
            x = torch.cat((box[i], x[i, j + 5, None], j[:, None].float(), embedding[i]), 1)
        else:  # best class only
            conf, j = x[:, 5:6].max(1, keepdim=True)
            x = torch.cat((box, conf, j.float(), embedding), 1)[conf.view(-1) > conf_thres]
...
def plot_test_images(images, targets, paths=None, fname='images.jpg', names=None, max_size=640, max_subplots=16):
    tl = 3  # line thickness
    tf = max(tl - 1, 1)  # font thickness
    if os.path.isfile(fname):  # do not overwrite
        return None
